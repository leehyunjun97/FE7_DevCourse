...
export default function App() {
  ...
  const [images, setImages] = useState(
    galleryImages.map((item) => ({ ...item, isDel: false }))
  );

  const delHandler = (index: number) => {
    setImages((prev) =>
      prev.map((item, i) => (i === index ? { ...item, isDel: true } : item))
    );
  };

  const recoverHandler = (index: number) => {
    setImages((prev) =>
      prev.map((item, i) => (i === index ? { ...item, isDel: false } : item))
    );
  };

  const recoverAllHandler = () => {
    setImages((prev) => prev.map((item) => ({ ...item, isDel: false })));
  };

  const delCount = () => {
    return images.filter((item) => item.isDel).length;
  };

  const recoverToggleView = () => {
    return !!images.filter((item) => item.isDel).length;
  };
      ...
      <footer className='text-center py-6 sm:py-8 px-4 sm:px-6 border-t border-border bg-muted/30'>
        <p className='text-muted-foreground text-sm sm:text-base'>
          ...
          <span className='block mt-2 text-xs sm:text-sm'>
            {delCount()} images hidden • {images.length - delCount()} displayed
          </span>
        </p>
      </footer>
    </main>
